...
class ArenaCommand(private val plugin: Charged) : CommandExecutor {
    ...
    override fun onCommand(
        ...
        when (args[0].lowercase()) {
            "create" -> {
                ...
                plugin.plugin.arenaManager.createArena(arena)
                sender.sendMessage("§aArena §f$name §acreada para modo §f$mode")
            }
            
            "list" -> {
                val arenas = plugin.plugin.arenaManager.getAllArenas()
            ...
            "delete" -> {
                ...
                val name = args[1]
                plugin.plugin.arenaManager.deleteArena(name)
